...
def load_data(path, model_type):
    ...
    for dataset in ["train", "test"]:
        ...
        logger.info("Loaded %d time series from %s" % (len(data[dataset]), fname))
...
def gluonts_fcast(cfg):   
    ...
    return {
        'loss'        : err_metrics['validate']['sMAPE'],
        'status'      : STATUS_OK,
        'cfg'         : cfg,
...
#    
#    space = {
#        'rand_seed' : rand_seed, # hp.choice('rand_seed', list(range(10000))),        
...
#    }
    ...
    space = {
        ...
        'model' : hp.choice('model', [
            {
                'type'                           : 'SimpleFeedForwardEstimator',
                'num_hidden_dimensions'          : hp.choice('num_hidden_dimensions', [[64, 32]]),
                'sff+max_epochs'                 : hp.choice('sff+max_epochs', [256]),
                'sff+num_batches_per_epoch'      : hp.choice('sff+num_batches_per_epoch', [64]),
                'sff+batch_size'                 : hp.choice('sff+batch_size', [64]),
                'sff+patience'                   : hp.choice('sff+patience', [8]),
                
                'sff+learning_rate'              : hp.choice('sff+learning_rate', [0.0023001638734668275]),
                'sff+learning_rate_decay_factor' : hp.choice('sff+learning_rate_decay_factor', [0.19438116390290547]),
                'sff+minimum_learning_rate'      : hp.choice('sff+minimum_learning_rate', [0.00009889247719672379]),
                'sff+weight_decay'               : hp.choice('sff+weight_decay', [1.0429215097094948e-8]),
                'sff+clip_gradient'              : hp.choice('sff+clip_gradient', [6.503501614688897]), 
            },
                  
            {
                ...
                'n_residue'                      : hp.choice('n_residue', [22]),
                'n_skip'                         : hp.choice('n_skip', [16]),
                'dilation_depth'                 : hp.choice('dilation_depth', [2]),
                'n_stacks'                       : hp.choice('n_stacks', [2]),
                'wn_act_type'                    : hp.choice('wn_act_type', ['relu']),
                
                'wn+max_epochs'                  : hp.choice('wn+max_epochs', [512]),
                'wn+num_batches_per_epoch'       : hp.choice('wn+num_batches_per_epoch', [256]),
                'wn+batch_size'                  : hp.choice('wn+batch_size', [32]),
                'wn+patience'                    : hp.choice('wn+patience', [16]),
                
                'wn+learning_rate'               : hp.choice('wn+learning_rate', [0.0009442729035727436]),
                'wn+learning_rate_decay_factor'  : hp.choice('wn+learning_rate_decay_factor', [0.13237161142224124]),
                'wn+minimum_learning_rate'       : hp.choice('wn+minimum_learning_rate', [0.000005422204857895246]),
                'wn+weight_decay'                : hp.choice('wn+weight_decay', [1.1760762943229199e-9]),
                'wn+clip_gradient'               : hp.choice('wn+clip_gradient', [1.0603375355482227]),
            },
                   
            {
                'type'                           : 'TransformerEstimator',
                'tf_use_xreg'                    : hp.choice('tf_use_xreg', [True]),
                'model_dim_heads'                : hp.choice('model_dim_heads', [[64, 2]]),
                'inner_ff_dim_scale'             : hp.choice('inner_ff_dim_scale', [2]),
                'pre_seq'                        : hp.choice('pre_seq', ['n']),
                'post_seq'                       : hp.choice('post_seq', ['r']),
                'tf_act_type'                    : hp.choice('tf_act_type', ['softrelu']),               
                'tf_dropout_rate'                : hp.choice('tf_dropout_rate', [0.01823868096830628]),
                
                'tf+max_epochs'                  : hp.choice('tf+max_epochs', [128]),
                'tf+num_batches_per_epoch'       : hp.choice('tf+num_batches_per_epoch', [256]),
                'tf+batch_size'                  : hp.choice('tf+batch_size', [128]),
                'tf+patience'                    : hp.choice('tf+patience', [8]),
                
                'tf+learning_rate'               : hp.choice('tf+learning_rate', [0.001935708609589899]),
                'tf+learning_rate_decay_factor'  : hp.choice('tf+learning_rate_decay_factor', [0.6499090641531957]),
                'tf+minimum_learning_rate'       : hp.choice('tf+minimum_learning_rate', [0.000005476544632160321]),
                'tf+weight_decay'                : hp.choice('tf+weight_decay', [1.3644488720985327e-8]),
                'tf+clip_gradient'               : hp.choice('tf+clip_gradient', [4.514210728417706]),
            },

            {
                ...
                'da_num_layers'                  : hp.choice('da_num_layers', [2]),
                'da_dropout_rate'                : hp.choice('da_dropout_rate', [0.01789919342318966]),
                
                'da+max_epochs'                  : hp.choice('da+max_epochs', [256]),
                ...
                'da+learning_rate'               : hp.choice('da+learning_rate', [0.003685551554639427]),
                'da+learning_rate_decay_factor'  : hp.choice('da+learning_rate_decay_factor', [0.16402392006124722]),
                'da+minimum_learning_rate'       : hp.choice('da+minimum_learning_rate', [0.000005728781254017914]),
                'da+weight_decay'                : hp.choice('da+weight_decay', [7.419786188658121e-9]),
                'da+clip_gradient'               : hp.choice('da+clip_gradient', [9.050305620105927]),
            }
        ])
    }
    ...
    if use_cluster:
        ...
        best = fmin(gluonts_fcast, space, rstate=np.random.RandomState(rand_seed), algo=rand.suggest, show_progressbar=False, trials=trials, max_evals=1000)
    else:
